...
with contextlib.redirect_stdout(open(os.devnull, 'w')):
    from app import (app, validate_aadhar, validate_mobile,
                     validate_court_order_date, ALLOWED_REFUND_STATUSES,
                     backfill_complaints, migrate_split_dbs_into_main, secure_base)
...
def login(client, role, username, password):
    ...
    return client.post('/login', data={'role': role, 'username': username,
                                        'password': password, 'csrf_token': tok})
...
with app.app_context():
    ...
    db.session.add(Transaction(ack_no='CASE-A', layer=1, from_account='111111111',
                               to_account='222222222', amount=100.0, txn_id='TX1', upload_id=uf.id))
    if not Complaint.query.filter_by(ack_no='CASE-A').first():
        db.session.add(Complaint(ack_no='CASE-A', file_name='caseA.xlsx',
                                 uploaded_by=A_ID, assigned_to=A_ID))
    db.session.commit()
...
check("anon POST /generate_letter -> not 200 (login required)",
      anon.post('/generate_letter', json={'ack_no': 'CASE-A', 'letter_type': 'suspect'}).status_code != 200)
check("non-owner B POST /generate_letter CASE-A -> 403",
      b_c.post('/generate_letter', json={'ack_no': 'CASE-A', 'letter_type': 'suspect'}).status_code == 403)
check("owner A POST /generate_letter CASE-A -> 200",
      a_c.post('/generate_letter', json={'ack_no': 'CASE-A', 'letter_type': 'suspect'}).status_code == 200)
check("non-owner B POST /generate_letter_docx CASE-A -> 403",
      b_c.post('/generate_letter_docx', json={'ack_no': 'CASE-A', 'account_number': '222222222',
                                              'letter_type': 'suspect'}).status_code == 403)
check("non-owner B POST /save_kyc for A's txn -> 403",
      b_c.post('/save_kyc', json={'txn_id': 'TX1', 'aadhar': '123412341234',
                                  'mobile': '9876543210'}).status_code == 403)
check("non-owner B POST /save_hold_refund CASE-A -> 403",
      b_c.post('/save_hold_refund', json={'ack_no': 'CASE-A', 'hold_txn_id': 'X',
                                          'refund_status': 'Refunded'}).status_code == 403)

print("\n── Admin-only gating ──")
check("officer GET /view_officers -> 403", a_c.get('/view_officers').status_code == 403)
check("officer POST /assign_case -> 403",
      a_c.post('/assign_case', data={'ack_no': 'CASE-A', 'officer_id': str(B_ID)}).status_code == 403)
check("officer GET /download_logs -> 403 (admin only, Bug #3)",
      a_c.get('/download_logs').status_code == 403)
check("admin GET /download_logs -> 200",
      admin_c.get('/download_logs').status_code == 200)
check("state_transactions tolerates junk pagination, no 500 (Bug #10)",
...
check("refund status whitelist enforced",
...
r = a_c.post('/save_kyc', json={'txn_id': 'TX1', 'aadhar': '123412341234',
                                'mobile': '9876543210', 'name': 'Synthetic', 'address': 'Test'})
...
with app.app_context():
    ...
    db.session.add(uf2); db.session.commit()
    db.session.add(Transaction(ack_no='LEGACY-1', layer=1, from_account='1',
                               to_account='2', amount=5.0, upload_id=uf2.id))
    db.session.commit()
    backfill_complaints()
...
_c.execute("CREATE TABLE poh_refund_details (id INTEGER PRIMARY KEY, ack_no TEXT, "
           "txn_id TEXT, court_order_date TEXT, refund_status TEXT, refund_amount REAL, updated_at TEXT)")
_c.execute("INSERT INTO poh_refund_details (ack_no, txn_id, refund_status) "
           "VALUES ('CASE-A', 'POH9', 'Refunded')")
_c.commit(); _c.close()
with app.app_context():
    ...
    check("old split file renamed to .migrated",
          os.path.exists(_poh_old + '.migrated') and not os.path.exists(_poh_old))
