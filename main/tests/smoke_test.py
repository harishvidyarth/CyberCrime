...
with app.app_context():
    db.create_all()
    for uname, role, pw in [('admin', 'Admin', 'AdminPass@2026!'),
        ...
        if u.id is None:
            db.session.add(u)
        u.role = role; u.set_password(pw); u.must_change_password = False
        u.failed_login_attempts = 0; u.account_locked_until = None
    db.session.commit()
...
print("\n── Auth & role routing ──")
admin = app.test_client(); officer = app.test_client()
check("admin login -> /admin_dashboard", 'admin_dashboard' in loc(login(admin, 'Admin', 'admin', 'AdminPass@2026!')))
check("officer login -> /index", '/index' in loc(login(officer, 'Investigative Officer', 'officer', 'OfficerPass@2026!')))
...
for p in ['/admin_dashboard', '/view_officers', '/manage_files', '/admin/add_officer',
          '/view_analytics', '/view_all_complaints']:
...
check("graph page is offline (no d3js.org / cdnjs)",
...
r = admin.post('/submit_officer', data={'name': 'T', 'rank': 'SI', 'email': 't@x.gov',
                                         'username': 'smoke_off', 'password': 'weak', 'csrf_token': tok})
check("weak pw is NOT 404", r.status_code != 404)
check("weak pw shows the rule", 'at least 12 characters' in r.get_data(as_text=True))
r = admin.post('/submit_officer', data={'name': 'T', 'rank': 'SI', 'email': 't@x.gov',
                                         'username': 'smoke_off', 'password': 'TempPass@2026!', 'csrf_token': tok})
check("strong pw -> /view_officers", 'view_officers' in loc(r))
with app.app_context():
    o = User.query.filter_by(username='smoke_off').first()
    check("officer saved, temp pw, must_change", bool(o) and o.must_change_password and o.check_password('TempPass@2026!'))

print("\n── CSRF & errors ──")
r = admin.post('/submit_officer', data={'name': 'X', 'rank': 'SI', 'email': 'x@x.gov', 'username': 'x2', 'password': 'TempPass@2026!'})
