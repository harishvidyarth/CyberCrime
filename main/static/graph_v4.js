...
function highlightHoldNode(accountNumber) {
  ...
  if (matchedNodes.length === 1 && svg) {
  ...
  } else if (matchedNodes.length > 1 && svg) {
    // Zoom and pan to fit all matched nodes in view!
    ...
    const dx = maxX - minX || 250;
    const dy = maxY - minY || 140;
    const x = (minX + maxX) / 2;
    const y = (minY + maxY) / 2;

    const padding = 120; // safe padding
    const scaleX = (width - padding * 2) / dx;
    const scaleY = (height - padding * 2) / dy;
    let scale = Math.min(scaleX, scaleY);
    scale = Math.max(0.12, Math.min(scale, 0.8)); // restrict bounds
...
function findAllPathsToAccount(root, accountNumber) {
  if (!root || !accountNumber) return [];
...
function expandHoldAccount(accountNumber) {
  const paths = findAllPathsToAccount(currentRoot, accountNumber);
...
function drawTree(root) {
  ...
  nodes.each(function (d) {
    ...
    console.log("Node:", d.data.name, "burst:", d.burst, "total_count:", d.burst_total_count, "children:", d.children?.length, "_children:", d._children?.length, "computed_total:", totalChildren);
